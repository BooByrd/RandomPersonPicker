...
namespace randomPersonGenerator
{
    class randomNumberGen
    {
        ...
        public List<int> randomNumber()
        {
            ...
            int minValue = 1;
            int maxValue = employeeList.Capacity + 1;
            #endregion

            #region Number of employee(s) draw
            Console.Write("\nEnter the number of employees to draw: ");
            try
            {
            ...
            while (minValue > amount || maxValue < amount)
            {
                Console.WriteLine($"\nAmount picked can't be lower than {minValue} and can't be higher than {maxValue}");
                Console.Write("\nEnter the number of employees to draw: ");
                try
                {
                    amount = Convert.ToInt32(Console.ReadLine());
